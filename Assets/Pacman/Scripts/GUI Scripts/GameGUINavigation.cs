...
public class GameGUINavigation : MonoBehaviour {
	...
	void Update () 
	{
		if(Input.GetKeyDown(KeyCode.Escape))
		{
			...
			if(PacmanGameManager.gameState == PacmanGameManager.GameState.Scores)
				Menu();
	...
	IEnumerator ShowReadyScreen(float seconds)
	{
		...
		PacmanGameManager.gameState = PacmanGameManager.GameState.Init;
		ReadyCanvas.enabled = true;
		yield return new WaitForSeconds(seconds);
		ReadyCanvas.enabled = false;
		PacmanGameManager.gameState = PacmanGameManager.GameState.Game;
	...
	public void getScoresMenu()
	{
		...
		PacmanGameManager.gameState = PacmanGameManager.GameState.Scores;
    ...
    IEnumerator AddScore(string name, int score)
    {
        ...
        if (ScorePost.error == null)
        {
            ...
            PacmanGameManager.score = 0;
            PacmanGameManager.Level = 0;
	...
	public void SubmitScores()
	{
	    ...
	    int highscore = PacmanGameManager.score;
    ...
    {
        PacmanGameManager.Level++;
		if (PlayerPrefs.GetInt("HIGH_LEVEL") < PacmanGameManager.Level)
		{
			PlayerPrefs.SetInt("HIGH_LEVEL", PacmanGameManager.Level);
		}
